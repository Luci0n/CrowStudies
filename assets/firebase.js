...
async function openAccountSettings(){
  var user=cloud.user;
  /* The account menu can appear from a remembered sign-in before the profile
     request finishes. Fetch once more here so a Google name can never replace
     the CrowStudies username in the settings card. */
  ...
  dialog.querySelector('[data-review-guide]').onchange=function(){ cloud.setReviewGuide(this.checked); };
  ...
  writeAuthHint(user); updateAuthControls(user);
  document.body.appendChild(shade);
}
function authHint(){
...
const cloud = {
  ...
  /* Whether the review explanation is shown at the start of a review. Kept
     beside the retention setting because it is the same kind of thing: a
     preference about reviewing that should follow the account between
     devices, and still work for somebody who is not signed in. */
  reviewGuide(){
    if (cloud.user && cloud.profile && typeof cloud.profile.reviewGuide === 'boolean') return cloud.profile.reviewGuide;
    try{ return localStorage.getItem('crowstudies:review-guide') !== 'off'; }catch(error){ return true; }
  },
  async setReviewGuide(on){
    var want = !!on;
    try{ localStorage.setItem('crowstudies:review-guide', want?'on':'off'); }catch(error){}
    if (!cloud.user) return;
    cloud.profile=Object.assign({},cloud.profile||{},{reviewGuide:want});
    try{ await setDoc(doc(db,'profiles',cloud.user.uid),{reviewGuide:want},{merge:true}); }catch(error){}
  },
  reviewRetention(){
